...
아래는 동일 실행의 터미널 로그 전문이다 (2026-09-21, `python graph.py`):
...
2. **`store/metrics.jsonl` 미구현**: 매 실행의 추출 성공률·소스별 기여 건수·소요 시간을 기록하는 로그가 아직 없다. 지금은 로그가 콘솔에만 남고 휘발된다.
...
4. **자동 실행 미구현**: 지금은 수동으로 `python graph.py`를 실행해야 한다. GitHub Actions로 매일 아침 스케줄링하는 작업이 남아있다.
